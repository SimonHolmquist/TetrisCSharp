...
public sealed class TestRotationSystem : IRotationSystem
{
    ...
    public IEnumerable<Coord> GetKickOffsets(TetrominoType type, Rotation from, Rotation to) => Offsets;

    public IReadOnlyList<Coord> GetOffsets(TetrominoType type, Rotation from, Rotation to) => Offsets;
}
